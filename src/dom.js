...
export function MakeContent(project) {
  ...
  projTitle.style.cssText =
    "border: none; color: rgba(255, 255, 255, 0.877); border-radius: 7px; background: rgb(24, 18, 43); padding: 2rem .5rem; height: 1ch; font-size: 3rem;";
  ...
  if (project.tasks.length == 0) {
    ...
    for (let i = 0; i < project.tasks.length; i++) {
      ...
      taskTitleDescCont.style.cssText =
        "display: flex; flex-direction: column; gap: 1rem;  margin-right: 1rem; width: 90%";
      ...
      taskTitle.style.cssText =
        "display: inline; width: fit-content; margin-left: 1rem; font-size: 1.5rem;";
      ...
      if (project.tasks[i].priority == "Urgent") {
        priorityText.style.cssText =
          "display: inline; color: red; margin-left: .5rem; opacity: 0.77; font-size: 1rem;";
      } else if (project.tasks[i].priority == "Important") {
        priorityText.style.cssText =
          "color: yellow; margin-left: .5rem; opacity: 0.77; font-size: 1rem;";
      } else if (project.tasks[i].priority == "Not Important") {
        priorityText.style.cssText =
          "color: green; margin-left: .5rem; opacity: 0.77; font-size: 1rem;";
      }
...
export function addTaskModal(project) {
  ...
  modalBackground.style.cssText =
    "position: fixed; z-index: 1; left: 0;  top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgb(0,0,0); background-color: rgba(0,0,0,0.4);";
  document.body.appendChild(modalBackground);

  let modalCont = document.createElement("div");
  modalCont.style.cssText =
    "margin: auto; display: flex; border: 1px solid #888; color: white; min-width: 500px; max-width: 800px; width: 70%; margin-top: 5rem; background-color: rgb(24, 18, 43); padding: 20px; justify-content: space-between; align-items: flex-start; font-size: 1.8rem; border-radius: 15px;";
  ...
  addTaskDescInput.style.cssText =
    "height: 100px; resize: vertical; padding: .5rem .5rem";

  exitBtn.textContent = "X";
  exitBtn.style.cssText =
    "padding: .2rem; background-color: rgb(24, 18, 43); color: white; font-weight: 600; cursor: pointer;";
  ...
  addTaskSubmit.style.cssText =
    "align-self: flex-start; padding: .5rem 1.5rem; font-weight: 600; cursor: pointer; border-radius: 15px;";
  form.appendChild(addTaskSubmit);

  addTaskSubmit.addEventListener("click", function (e) {
    e.preventDefault();
    modalBackground.style.display = "none";
    let addingTask = new Task(
      addTitleInput.value,
...
export function editTaskModal(project, iterable) {
  ...
  editTaskDescInput.style.cssText =
    "height: 100px; resize: vertical; padding: .5rem .5rem";
